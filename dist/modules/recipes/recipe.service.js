"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RecipeService = void 0;
const common_1 = require("@nestjs/common");
const sequelize_1 = require("@nestjs/sequelize");
const models_1 = require("./models");
const model_1 = require("../meal/model");
const product_1 = require("../product");
let RecipeService = class RecipeService {
    constructor(recipeModel, mealModel, productModel) {
        this.recipeModel = recipeModel;
        this.mealModel = mealModel;
        this.productModel = productModel;
    }
    async create(createRecipeDto) {
        const meal = await this.mealModel.findByPk(createRecipeDto.meal_id);
        const product = await this.productModel.findByPk(createRecipeDto.product_id);
        if (!meal || !product) {
            throw new common_1.NotFoundException('Meal or Product not found');
        }
        const recipe = await this.recipeModel.create({
            ...createRecipeDto,
            meal_id: meal.id,
            product_id: product.id,
        });
        return recipe;
    }
    async findAll() {
        return this.recipeModel.findAll({
            include: [model_1.Meal, product_1.Product],
        });
    }
    async findOne(id) {
        const recipe = await this.recipeModel.findByPk(id, {
            include: [model_1.Meal, product_1.Product],
        });
        if (!recipe) {
            throw new common_1.NotFoundException(`Recipe with id ${id} not found`);
        }
        return recipe;
    }
    async update(id, updateRecipeDto) {
        const recipe = await this.findOne(id);
        if (!recipe) {
            throw new common_1.NotFoundException(`Recipe with id ${id} not found`);
        }
        await recipe.update(updateRecipeDto);
        return recipe;
    }
    async remove(id) {
        const recipe = await this.findOne(id);
        if (!recipe) {
            throw new common_1.NotFoundException(`Recipe with id ${id} not found`);
        }
        await recipe.destroy();
    }
};
exports.RecipeService = RecipeService;
exports.RecipeService = RecipeService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, sequelize_1.InjectModel)(models_1.Recipe)),
    __param(1, (0, sequelize_1.InjectModel)(model_1.Meal)),
    __param(2, (0, sequelize_1.InjectModel)(product_1.Product)),
    __metadata("design:paramtypes", [Object, Object, Object])
], RecipeService);
//# sourceMappingURL=recipe.service.js.map